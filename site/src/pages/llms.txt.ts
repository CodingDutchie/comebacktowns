...
export const GET: APIRoute = () => {
  ...
  const lines: string[] = [
    ...
    "",
    `Versions: readiness ${first?.config_version ?? "n/a"} computed ${first?.computed_at.slice(0, 10) ?? "n/a"}; momentum ${firstMo?.config_version ?? "n/a"} computed ${firstMo?.computed_at.slice(0, 10) ?? "n/a"}; published ${stamp}. ${site.SITE_NAME} is independent: no municipality, developer or broker, and no listings. Corrections: ${site.CONTACT_EMAIL}.`,
    "",
